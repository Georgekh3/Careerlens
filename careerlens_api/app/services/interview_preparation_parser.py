...
from app.schemas.interview_preparation import InterviewPreparationResult
from app.schemas.profile import StructuredProfile


class InterviewPreparationParser:
    def prepare(
        self,
        *,
                    ...
                    "content": [
                        {
                            "type": "input_text",
                            "text": (
                                "You are an interview coach. Generate focused interview preparation "
                                "based on the candidate profile and the target job description. "
                                "Return only structured JSON matching the schema. "
                                "Be specific, realistic, and grounded in the provided data."
                            ),
                        }
                    ],
                },
        ...
        if not response.ok:
            raise RuntimeError(
                f"OpenAI interview preparation error {response.status_code}: {response.text}"
            )

        response_json = response.json()
        parsed_json = self._extract_output_json(response_json)
        return InterviewPreparationResult.model_validate(parsed_json)
